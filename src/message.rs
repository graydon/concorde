...
impl<ObjLD: LatticeDef,
    ...
    std::cmp::PartialEq
    for Message<ObjLD,Peer>
    where ObjLD::T : Clone+Debug+Default+Eq
{
    fn eq(&self, other: &Self) -> bool
    {
        match (self, other)
        {
            (Message::Request{seq: seq1, from: from1, to: to1, opinion: opinion1},
                Message::Request{seq: seq2, from: from2, to: to2, opinion: opinion2}) =>
                {
                    let tup1 = (seq1, from1, to1, opinion1);
                    let tup2 = (seq2, from2, to2, opinion2);
                    tup1.eq(&tup2)
                }
            (Message::Response{seq: seq1, from: from1, to: to1, opinion: opinion1},
                Message::Response{seq: seq2, from: from2, to: to2, opinion: opinion2}) =>
                {
                    let tup1 = (seq1, from1, to1, opinion1);
                    let tup2 = (seq2, from2, to2, opinion2);
                    tup1.eq(&tup2)
                }
            (Message::Commit{from: from1, state: state1},
                Message::Commit{from: from2, state: state2}) =>
                {
...
std::cmp::PartialOrd
for Message<ObjLD,Peer>
    where ObjLD::T : Clone+Debug+Default+Ord
{
