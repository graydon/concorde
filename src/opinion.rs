...
use crate::{StateLE,CfgLE,StateLEExt};
use pergola::{LatticeElt,LatticeDef};
...
where ObjLD::T : Clone+Debug+Default
{
    pub fn same_estimated_commit_config(&self,
                                        other: &Self) -> bool
    {
        self.estimated_commit.config() ==
            other.estimated_commit.config()
    }
    pub fn same_estimated_and_proposed_configs(&self,
                                               other: &Self) -> bool
    {
    ...
    where ObjLD::T : Clone+Debug+Default+Eq
{
    ...
    {
        let tup1 = (&self.estimated_commit,&self.proposed_configs,&self.candidate_object);
        let tup2 = (&other.estimated_commit,&other.proposed_configs,&other.candidate_object);
        tup1.eq(&tup2)
    }
}
